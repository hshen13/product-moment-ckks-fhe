...
Row RunOne(const Runtime& rt, const std::string& mode, double keepProb, bool safeBootstrap, uint32_t dataSeed,
    ...
    try {
        ...
        if (productPower >= 2) {
            pt3 = cc->MakeCKKSPackedPlaintext(x3, 1, rt.depth - 1);
            pt3->SetLength(sampleCount);
            ct3 = cc->Encrypt(kp.publicKey, pt3);
        }
        if (productPower >= 3) {
            pt4 = cc->MakeCKKSPackedPlaintext(x4, 1, rt.depth - 1);
        ...
        if (productPower >= 2) {
            h3 = cc->EvalBootstrap(ct3);
        }
        if (productPower >= 3) {
            h4 = cc->EvalBootstrap(ct4);
        }
        ...
        if (!stageOut.empty()) {
            AppendStageRow(stageOut, "cpu", mode, dataSeed, "h1", DecryptReal(cc, kp.secretKey, h1, sampleCount), x1);
            AppendStageRow(stageOut, "cpu", mode, dataSeed, "h2", DecryptReal(cc, kp.secretKey, h2, sampleCount), x2);
            if (productPower >= 2) {
                AppendStageRow(stageOut, "cpu", mode, dataSeed, "h3", DecryptReal(cc, kp.secretKey, h3, sampleCount), x3);
            }
            if (productPower >= 3) {
                AppendStageRow(stageOut, "cpu", mode, dataSeed, "h4", DecryptReal(cc, kp.secretKey, h4, sampleCount), x4);
            }
            AppendStageRow(stageOut, "cpu", mode, dataSeed, "inter", DecryptReal(cc, kp.secretKey, inter, sampleCount),
                           refInter);
            AppendStageRow(stageOut, "cpu", mode, dataSeed, "logit", out, refLogits);
        }
        row.finite = true;
        for (double v : out) {
            if (!std::isfinite(v)) {
